...
pub async fn redirect(
    ...
    let referer_header = headers
        .get("referer")
        .map(|header_value| header_value.to_str().unwrap_or_default().to_string());
    ...
    let saved_statistics = tokio::time::timeout(
        ...
        sqlx::query(
            r#"
            insert into statistics (link_id, referer, user_agent)
            values ($1, $2, $3)
            "#,
        )
        .bind(&requested_link)
    ...
    match saved_statistics {
        ...
        _ => {
            tracing::debug!("saved statistics for link {}", link.id);
        }
    }
...
pub async fn update_link(
    ...
    tracing::debug!("updated link {} with target {}", link.id, link.target_url);
